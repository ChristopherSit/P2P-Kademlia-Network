...
function handleClient(sock, serverDHTtable) {
  //insert check for image request here since if started as client this function is called eventually to handle server behavior
  let kadPacket = null;
  ...
  sock.on('data', (message) => {
    //check if kadPacket or Image packet image packet if 5-24rd bit are all 0s
    if(ImageHandler.isImagePacket(message)) {
      imageFlag = true;
      ImageHandler.handleImageRequest(message, serverDHTtable, sock);
      clientSocket = sock;
    } 

    //handle search packet
    else if (ImageHandler.isSearchPacket(message, serverDHTtable)) {
      imageFlag = true;
      ImageHandler.handleSearchRequest(message, serverDHTtable);
  
    } 

    //handle found packet 
    else if (ImageHandler.isFoundPacket(message)) {
      imageFlag = true;
      ImageHandler.handleFoundPacket(message, clientSocket)
    } 
    
    // handle asn2
    else {
      imageFlag = false;
      //regular behavior here
  ...
  //will need to implement new behavior since we dont want a client request socket being added here
  //Implemented imageFlag check seems to work

    // client edded the connection
    if (kadPacket && !imageFlag) {
